...
exports.FinP2PSecurityRobustnessBenchmark = void 0;
const events_1 = require("events");
const logger_1 = require("../../../dist/core/utils/logger");
const FinP2PSDKRouter_1 = require("../../../dist/core/router/FinP2PSDKRouter");
const FinP2PIntegratedSuiAdapter_1 = require("../../../dist/adapters/finp2p/FinP2PIntegratedSuiAdapter");
const FinP2PIntegratedHederaAdapter_1 = require("../../../dist/adapters/finp2p/FinP2PIntegratedHederaAdapter");
const port_scanner_1 = require("../../../dist/core/utils/port-scanner");
...
class FinP2PSecurityRobustnessBenchmark extends events_1.EventEmitter {
    constructor() {
        super();
    ...
    async runFormalVerificationTests() {
        ...
        try {
            ...
            if (valueConservationResult.status === 'PASSED')
                testResult.metrics.passedTests++;
            else
                testResult.metrics.failedTests++;
            ...
            if (finalizationResult.status === 'PASSED')
                testResult.metrics.passedTests++;
            else
                testResult.metrics.failedTests++;
            ...
            if (idempotencyResult.status === 'PASSED')
                testResult.metrics.passedTests++;
            else
                testResult.metrics.failedTests++;
            // Calculate score
    ...
    async testReplayRejection() {
        ...
        try {
            ...
            const firstSuiTransfer = await this.suiAdapter?.transferByFinId('security-test-account1@finp2p.test', 'security-test-account2@finp2p.test', transferAmount, true);
            if (firstSuiTransfer) {
                this.emit('progress', { message: '   Step 2: First atomic swap - HBAR transfer' });
                const firstHbarTransfer = await this.hederaAdapter?.transferByFinId('security-test-account2@finp2p.test', 'security-test-account1@finp2p.test', hbarAmount, true);
                artifacts.push({
            ...
            try {
                ...
                const secondSuiTransfer = await this.suiAdapter?.transferByFinId('security-test-account1@finp2p.test', 'security-test-account2@finp2p.test', transferAmount, true);
                if (secondSuiTransfer) {
                    this.emit('progress', { message: '   Step 4: Second atomic swap - HBAR transfer' });
                    const secondHbarTransfer = await this.hederaAdapter?.transferByFinId('security-test-account2@finp2p.test', 'security-test-account1@finp2p.test', hbarAmount, true);
    ...
    async testValueConservation() {
        ...
        try {
            ...
            for (let i = 0; i < atomicSwapCount; i++) {
                ...
                const suiTransfer = await this.suiAdapter?.transferByFinId('security-test-account1@finp2p.test', 'security-test-account2@finp2p.test', suiAmount, true);
                if (suiTransfer) {
                    totalSuiDebits += suiAmount;
                    totalSuiCredits += suiAmount;
                    this.emit('progress', { message: `   Atomic swap ${i + 1}: HBAR transfer` });
                    const hbarTransfer = await this.hederaAdapter?.transferByFinId('security-test-account2@finp2p.test', 'security-test-account1@finp2p.test', hbarAmount, true);
                    if (hbarTransfer) {
    ...
    async testNoPrematureFinalization() {
        let violations = 0;
        const artifacts = [];
        try {
    ...
    async testIdempotencyUnderRetries() {
        let violations = 0;
        const artifacts = [];
        try {
            ...
            const promises = Array(2).fill(null).map(async (_, index) => {
                try {
                    ...
                    // Perform atomic swap
                    const suiResult = await this.suiAdapter?.transferByFinId('security-test-account1@finp2p.test', 'security-test-account2@finp2p.test', suiAmount, true);
                    if (suiResult) {
                        const hederaResult = await this.hederaAdapter?.transferByFinId('security-test-account2@finp2p.test', 'security-test-account1@finp2p.test', hbarAmount, true);
                        return {
            ...
            const uniqueSuiTxHashes = new Set(successfulAtomicSwaps.map(t => t.suiTxHash));
            const uniqueHederaTxIds = new Set(successfulAtomicSwaps.map(t => t.hederaTxId));
            ...
            artifacts.push({
                ...
                atomicSwaps: successfulAtomicSwaps.map(t => ({
                    suiTxHash: t.suiTxHash,
                    hederaTxId: t.hederaTxId,
                    index: t.index,
            ...
            if (uniqueSuiTxHashes.size < successfulAtomicSwaps.length || uniqueHederaTxIds.size < successfulAtomicSwaps.length) {
                ...
                artifacts.push({
                    ...
                    uniqueSuiCount: uniqueSuiTxHashes.size,
                    uniqueHederaCount: uniqueHederaTxIds.size,
                    suiTxHashes: successfulAtomicSwaps.map(t => t.suiTxHash),
                    hederaTxIds: successfulAtomicSwaps.map(t => t.hederaTxId),
    ...
    async runCryptographicRobustnessTests() {
    ...
    async testSenderAuthenticity() {
        let mismatches = 0;
        const artifacts = [];
        try {
            // Perform a real transfer and verify sender authenticity
            const transfer = await this.suiAdapter?.transferByFinId('security-test-account1@finp2p.test', 'security-test-account2@finp2p.test', BigInt(1000000), true);
    ...
    async testDomainSeparation() {
        let crossChainReplays = 0;
        const artifacts = [];
        try {
            // Test domain separation by attempting cross-chain transfers
            // First, perform a Sui transfer
            const suiTransfer = await this.suiAdapter?.transferByFinId('security-test-account1@finp2p.test', 'security-test-account2@finp2p.test', BigInt(1000000), true);
            if (suiTransfer) {
                artifacts.push({
                    type: 'sui_transfer',
            ...
            try {
                const hederaTransfer = await this.hederaAdapter?.transferByFinId('security-test-account1@finp2p.test', 'security-test-account2@finp2p.test', BigInt(10000000), // 0.1 HBAR in tinybars
                true);
                if (hederaTransfer) {
                    artifacts.push({
    ...
    async testTamperCheck() {
        let tamperedAccepted = 0;
        const artifacts = [];
        try {
            // Perform a valid transfer first
            const validTransfer = await this.suiAdapter?.transferByFinId('security-test-account1@finp2p.test', 'security-test-account2@finp2p.test', BigInt(1000000), true);
            if (validTransfer) {
                artifacts.push({
                    type: 'valid_transfer',
            ...
            try {
                const tamperedTransfer = await this.suiAdapter?.transferByFinId('security-test-account1@finp2p.test', 'malicious-account@finp2p.test', // Invalid recipient
                BigInt(1000000), true);
                if (tamperedTransfer) {
                    tamperedAccepted++;
                    artifacts.push({
    ...
    async runHSMKMSupportTests() {
        ...
        try {
            ...
            if (rotationResult.status === 'PASSED')
                testResult.metrics.passedTests++;
            else
                testResult.metrics.failedTests++;
            ...
            if (revocationResult.status === 'PASSED')
                testResult.metrics.passedTests++;
            else
                testResult.metrics.failedTests++;
            // Calculate score
    ...
    async testKeyRotation() {
        ...
        try {
            ...
            const beforeTransfer = await this.suiAdapter?.transferByFinId('security-test-account1@finp2p.test', 'security-test-account2@finp2p.test', BigInt(1000000), true);
            if (beforeTransfer) {
                artifacts.push({
            ...
            const afterTransfer = await this.suiAdapter?.transferByFinId('security-test-account1@finp2p.test', 'security-test-account2@finp2p.test', BigInt(1000000), true);
            if (afterTransfer) {
                artifacts.push({
    ...
    async testPostRevocationAcceptance() {
    ...
    async runByzantineFaultToleranceTests() {
        ...
        try {
            ...
            if (finalityResult.status === 'PASSED')
                testResult.metrics.passedTests++;
            else
                testResult.metrics.failedTests++;
    ...
    async testFinalityThresholdConformance() {
    ...
    async runVulnerabilityAssessmentTests() {
        ...
        try {
            ...
            if (containerResult.status === 'PASSED')
                testResult.metrics.passedTests++;
            else
                testResult.metrics.failedTests++;
            // Calculate score
    ...
    calculateFinalResults() {
        ...
        // Count evidence
        this.results.evidence.logsCollected = this.results.criteria.reduce((sum, criterion) => sum + criterion.artifacts.filter(a => a.type.includes('log')).length, 0);
        this.results.evidence.metricsCollected = this.results.criteria.reduce((sum, criterion) => sum + criterion.artifacts.filter(a => a.type.includes('metric')).length, 0);
        this.results.evidence.tracesCollected = this.results.criteria.reduce((sum, criterion) => sum + criterion.artifacts.filter(a => a.type.includes('trace')).length, 0);
...
exports.FinP2PSecurityRobustnessBenchmark = FinP2PSecurityRobustnessBenchmark;
// Main execution
async function main() {
