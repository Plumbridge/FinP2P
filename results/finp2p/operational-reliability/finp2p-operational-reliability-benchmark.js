...
exports.FinP2POperationalReliabilityBenchmark = void 0;
...
class FinP2POperationalReliabilityBenchmark extends events_1.EventEmitter {
    constructor() {
        super();
    ...
    async initializeFinP2P() {
        this.emit('progress', { message: '🔧 Initializing FinP2P components...' });
        // Find available port
        const port = await (0, port_scanner_1.findAvailablePort)(3000);
    ...
    async runObservabilityReadinessTests() {
        ...
        try {
            ...
            if (observabilityResult.status === 'PASSED')
    ...
    async testObservabilityReadiness() {
        const artifacts = [];
        try {
            ...
            const startTime = Date.now();
            const suiTransfer = await this.suiAdapter?.transferByFinId('operational-test-account1@finp2p.test', 'operational-test-account2@finp2p.test', suiAmount, true);
            if (suiTransfer) {
                const hbarTransfer = await this.hederaAdapter?.transferByFinId('operational-test-account2@finp2p.test', 'operational-test-account1@finp2p.test', hbarAmount, true);
                const endTime = Date.now();
            ...
            try {
                ...
                await this.suiAdapter?.transferByFinId('operational-test-account1@finp2p.test', 'operational-test-account2@finp2p.test', invalidAmount, true);
            }
            catch (error) {
                artifacts.push({
            ...
            artifacts.forEach(artifact => {
                ...
                if (artifact.timestamp)
                    score++;
                if (artifact.correlationId)
                    score++;
                if (artifact.result)
                    score++;
                if (artifact.suiTxHash || artifact.hederaTxId || artifact.error)
                    score++;
                if (artifact.latency || artifact.error)
                    score++;
    ...
    async runFaultRecoveryCapabilitiesTests() {
    ...
    async testFaultRecoveryCapabilities() {
        const artifacts = [];
        try {
            ...
            let hederaTransferPromise;
            try {
                hederaTransferPromise = this.hederaAdapter?.transferByFinId('operational-test-account1@finp2p.test', 'operational-test-account2@finp2p.test', hbarAmount1, true);
            }
            catch (error) {
            ...
            try {
                ...
                if (hederaTransfer1) {
                    const hederaTransfer2 = await this.hederaAdapter?.transferByFinId('operational-test-account2@finp2p.test', 'operational-test-account1@finp2p.test', hbarAmount2, true);
                    artifacts.push({
            ...
            const port2 = await (0, port_scanner_1.findAvailablePort)(3000);
            this.finp2pRouter = new FinP2PSDKRouter_1.FinP2PSDKRouter({
                port: port2,
                routerId: process.env.ROUTER_ID || process.env.FINP2P_ROUTER_ID || 'operational-benchmark-router',
                orgId: process.env.FINP2P_ORG_ID || 'operational-benchmark-org',
            ...
            try {
                const testHbarTransfer1 = await this.hederaAdapter?.transferByFinId('operational-test-account1@finp2p.test', 'operational-test-account2@finp2p.test', BigInt(500000), // Hedera amount
                true);
                if (testHbarTransfer1) {
                    const testHbarTransfer2 = await this.hederaAdapter?.transferByFinId('operational-test-account2@finp2p.test', 'operational-test-account1@finp2p.test', BigInt(300000), // Hedera amount
                    true);
                    artifacts.push({
                        type: 'idle_recovery',
    ...
    async runLifecycleManagementProcessTests() {
    ...
    async testLifecycleManagementProcess() {
        const artifacts = [];
        try {
            ...
            const oldPort = this.finp2pRouter.port;
            const newPort = await (0, port_scanner_1.findAvailablePort)(3001);
            // Stop current router
            await this.finp2pRouter?.stop();
            ...
            const hbarTransfer1 = await this.hederaAdapter?.transferByFinId('operational-test-account1@finp2p.test', 'operational-test-account2@finp2p.test', BigInt(750000), true);
            if (hbarTransfer1) {
                const hbarTransfer2 = await this.hederaAdapter?.transferByFinId('operational-test-account2@finp2p.test', 'operational-test-account1@finp2p.test', BigInt(750000), true);
...
exports.FinP2POperationalReliabilityBenchmark = FinP2POperationalReliabilityBenchmark;
