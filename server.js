...
function getDemoFilePath(demoType) {
    ...
    const demoFiles = {
        'atomic-swap': 'demos/finp2p-atomic-swap-coordination-demo.js',
        'hedera-sui': 'demos/hedera-sui-adapters-demo.js',
        'fusion': 'demos/fusion-multi-chain-demo.js'
    };
...
function parseDemoOutput(demoType, output) {
    ...
    for (const line of lines) {
        ...
        } else if (trimmedLine.includes('🧪 FUSION API v1.0.0 ENDPOINT TESTS') || 
                   trimmedLine.includes('🔄 REAL CROSS-CHAIN TRANSFERS') ||
            ...
            if (trimmedLine.includes('SUI Transfer:')) {
                ...
                for (const pattern of patterns) {
                    ...
                    if (match && match[1] && match[1] !== 'undefined') {
                        ...
                        if (trimmedLine.includes('Real transaction executed successfully') || 
                        ...
                        if (txType === 'Unknown') {
                            ...
                            if (txId.includes('@')) {
                                txType = 'HBAR';
                            } else if (txId.length > 40 && /^[A-Za-z0-9]+$/.test(txId)) {
                                // SUI transaction hashes are long alphanumeric strings
                                txType = 'SUI';
        ...
        if (isInBalanceSection && 
            ...
            let balanceMatch = trimmedLine.match(/(\w+)\s+(SUI|HBAR|ETH)\s+Balance:\s+([0-9.]+)\s+(SUI|HBAR|ETH)(?:\s+\(Change:\s+([+-]?[0-9.-]+)\))?/);
            
            // If no match, try alternative format for Fusion demo with leading spaces
            if (!balanceMatch) {
                balanceMatch = trimmedLine.match(/\s*(\w+)\s+(SUI|HBAR|ETH)\s+Balance:\s+([0-9.]+)\s+(SUI|HBAR|ETH)(?:\s+\(Change:\s+([+-]?[0-9.-]+)\))?/);
            }
            ...
            if (!balanceMatch) {
                balanceMatch = trimmedLine.match(/\d{2}:\d{2}:\d{2}\s+\[info\]:\s*(\w+)\s+(SUI|HBAR|ETH)\s+Balance:\s*([0-9.]+)\s*(SUI|HBAR|ETH)(?:\s*\(Change:\s*([+-]?[0-9.-]+)\))?/);
            }
            ...
            if (balanceMatch) {
                ...
                result.balances[tokenType] = amount;
            } else {
                console.log(`❌ Balance parsing failed for line: "${trimmedLine}"`);
            }
        }
        ...
        // Extract endpoint results for Fusion demo
        if (demoType === 'fusion' && trimmedLine.includes('=') && 
            ...
            if (match) {
                const [, endpoint, resultText] = match;
                if (!result.endpointResults) {
                    result.endpointResults = {};
                }
                result.endpointResults[endpoint] = resultText.trim();
            }
        }
