...
const MockAdapter_1 = require("../../src/adapters/MockAdapter");
const LedgerManager_1 = require("../../src/router/LedgerManager");
...
describe('Enhanced Balance Tracking', () => {
    ...
    beforeEach(async () => {
        ...
        ledgerManager = new LedgerManager_1.LedgerManager(ledgerConfig, logger);
    ...
    describe('MockAdapter Enhanced Features', () => {
        ...
        it('should simulate insufficient balance scenarios', async () => {
            ...
            await expect(mockAdapter.transfer(testAccount1, testAccount2, testAssetId, BigInt('1000000000000000000'))).rejects.toThrow('Insufficient balance');
        });
        it('should simulate network partitions', async () => {
            mockAdapter.simulateNetworkPartition(true);
            await expect(mockAdapter.getBalance(testAccount1, testAssetId)).rejects.toThrow('Network partition');
    ...
    describe('LedgerManager Enhanced Features', () => {
        it('should validate balance availability', async () => {
            const validation = await ledgerManager.validateBalanceAvailability('mock', testAccount1, testAssetId, BigInt('50000000000000000000'));
        ...
        it('should reserve and release balance', async () => {
            const reservation = await ledgerManager.reserveBalance('mock', testAccount1, testAssetId, BigInt('30000000000000000000'));
            expect(reservation.success).toBe(true);
            expect(reservation.reservationId).toBeDefined();
            // Check that reserved amount affects availability
            const validation = await ledgerManager.validateBalanceAvailability('mock', testAccount1, testAssetId, BigInt('80000000000000000000'));
            ...
            const validationAfter = await ledgerManager.validateBalanceAvailability('mock', testAccount1, testAssetId, BigInt('80000000000000000000'));
            expect(validationAfter.available).toBe(true);
        });
        it('should lock and unlock reserved balance', async () => {
            const reservation = await ledgerManager.reserveBalance('mock', testAccount1, testAssetId, BigInt('25000000000000000000'));
            expect(reservation.success).toBe(true);
        ...
        it('should handle cross-ledger transfer initiation', async () => {
            ...
            const crossTransfer = await ledgerManager.initiateCrossLedgerTransfer('mock', 'mock', // Same ledger for testing
            testAccount1, testAccount2, testAssetId, BigInt('20000000000000000000'));
        ...
        it('should rollback cross-ledger operations', async () => {
            const crossTransfer = await ledgerManager.initiateCrossLedgerTransfer('mock', 'mock', testAccount1, testAccount2, testAssetId, BigInt('15000000000000000000'));
            expect(crossTransfer.success).toBe(true);
            ...
            // Check that balance is fully available again
            const validation = await ledgerManager.validateBalanceAvailability('mock', testAccount1, testAssetId, BigInt('100000000000000000000'));
            expect(validation.available).toBe(true);
        });
        it('should handle insufficient balance scenarios gracefully', async () => {
            // Try to reserve more than available
            const reservation = await ledgerManager.reserveBalance('mock', testAccount1, testAssetId, BigInt('150000000000000000000') // More than the 100 tokens available
            );
        ...
        it('should clean up expired reservations', async () => {
            ...
            const reservation = await ledgerManager.reserveBalance('mock', testAccount1, testAssetId, BigInt('10000000000000000000'));
            expect(reservation.success).toBe(true);
            // Wait for expiration
            await new Promise(resolve => setTimeout(resolve, 200));
            // Trigger cleanup manually (normally done by timer)
            ledgerManager.cleanupExpiredReservations();
    ...
    describe('Integration Scenarios', () => {
        it('should handle high-volume transfers with proper balance tracking', async () => {
            ...
            for (let i = 0; i < transferCount; i++) {
                promises.push(mockAdapter.transfer(testAccount1, testAccount2, testAssetId, transferAmount));
            }
        ...
        it('should maintain consistency under concurrent operations', async () => {
            ...
            for (let i = 0; i < concurrentOps; i++) {
                reservationPromises.push(ledgerManager.reserveBalance('mock', testAccount1, testAssetId, operationAmount));
            }
            ...
            const validation = await ledgerManager.validateBalanceAvailability('mock', testAccount1, testAssetId, BigInt('150000000000000000000') // Should fail due to reservations
            );
            expect(validation.available).toBe(false);
            if (!validation.available) {
                expect(validation.reason).toBeDefined();
            ...
            const finalValidation = await ledgerManager.validateBalanceAvailability('mock', testAccount1, testAssetId, BigInt('200000000000000000000'));
            expect(finalValidation.available).toBe(true);
        });
    });
});
//# sourceMappingURL=BalanceTracking.test.js.map