...
// Fusion API Types (v1.0.0-compatible)
...
export interface FusionTransferOrigin {
  originId: string; // Can be FinID (account1@fusion.demo) or native address (0x1234...)
}

export interface FusionTransferDestination {
  destinationId: string; // Can be FinID (account2@fusion.demo) or native address (0x1234...)
 ...
 * 
 * Implements the Fusion API with FinP2P integration
 ...
 * - Fusion API compliance (create proposals → sign → execute)
...
export class FinP2PIntegratedFusionAdapter extends EventEmitter {
  ...
  /**
   * Create a transfer proposal (Fusion API)
  ...
  /**
   * Create a smart contract write proposal (Fusion API)
   */
  ...
  /**
   * Create a smart contract deployment proposal (Fusion API)
   */
  ...
  /**
   * Execute a signed transaction (Fusion API)
   */
  ...
  private async buildSmartContractWriteTransaction(
    ...
    try {
      ...
      this.logger.info('Gas estimation unavailable for write; using default with fallback buffer.', { reason: (error instanceof Error ? error.message : String(error)) });
    }
  ...
  private async buildSmartContractDeployTransaction(
    ...
    try {
      ...
      this.logger.info('Gas estimation unavailable; using conservative default. Reason:', errorMessage);
  ...
  async readSmartContract(request: FusionSmartContractReadRequest): Promise<FusionEVMSmartContractResponse> {
    try {
      ...
      // Step 2: Build ABI with explicit return types (improves decode reliability)
      const inputTypes = request.contractDetails.inputParameters.map(p => p.type).join(',');
      const outputTypes = request.contractDetails.outputParameters.map(p => p.type).join(',');
      const abiFragment = `function ${request.contractDetails.functionName}(${inputTypes}) view returns (${outputTypes})`;
  ...
  private decodeSmartContractReturns(
    ...
    try {
      ...
      const decoded = iface.decodeFunctionResult(functionName, rawValue);
