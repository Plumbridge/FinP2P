...
export class FinP2PSDKRouter extends EventEmitter {
  ...
  private initializeMockData(): void {
    this.logger.info('🎭 Initializing mock data (mock mode)');
    ...
    // Get second account addresses for Account 2 (different from Account 1)
    const account2SuiAddress = process.env.SUI_ADDRESS_2 || realSuiAddress || '0x000...account2_sui_not_available';
    const account2HederaAccount = process.env.HEDERA_ACCOUNT_ID_2 || realHederaAccountId;
    
    // TRUE CROSS-CHAIN SWAP SETUP: Account 1 has SUI (wants HBAR), Account 2 has HBAR (wants SUI)
    // This demonstrates real cross-party asset exchange via FinP2P coordination
    const account1SuiAddress = realSuiAddress || '0x000...account1_sui_not_available'; // Account 1 has SUI to trade
    const account1HederaAccount = realHederaAccountId; // Account 1 wants to receive HBAR here
    
    this.logger.info('🔄 TRUE CROSS-CHAIN ASSET EXCHANGE SETUP:');
    this.logger.info('   • Account 1: Has SUI tokens, wants HBAR tokens');
    this.logger.info('   • Account 2: Has HBAR tokens, wants SUI tokens');
    this.logger.info('   • FinP2P will coordinate cross-chain asset exchange');
    this.logger.info('   • Account 1 trades her SUI for Account 2\'s HBAR via atomic swap protocol!');
    
    // Mock users configured for proper atomic swap
    const mockUsers = [
      { 
        finId: 'finp2p_account1_123',
        name: 'Account 1 Johnson',
        email: 'account1@demo.com',
        wallets: {
          sui: '0x123abc...account1_sui',
          hedera: '0.0.123456',
          ethereum: '0x456def...account1_eth'
        }
      },
      { 
        finId: 'finp2p_account2_456',
        name: 'Account 2 Smith', 
        email: 'account2@demo.com',
        wallets: {
          sui: '0x789xyz...account2_sui',
          hedera: '0.0.789012',
          ethereum: '0x123ghi...account2_eth'
        }
      },
      // Account 1: Has SUI, wants HBAR - will trade her SUI for Account 2's HBAR
      { 
        finId: 'account1@atomic-swap.demo',
        name: 'Account 1 (Trader: SUI → HBAR)',
        email: 'account1@atomic-swap.demo',
        wallets: {
          sui: account1SuiAddress, // Account 1's SUI wallet (has SUI to trade)
          hedera: account1HederaAccount, // Account 1's HBAR wallet (wants to receive HBAR)
          ethereum: process.env.SEPOLIA_WALLET_ADDRESS || '0x000...placeholder'
        }
      },
      // Account 2: Has HBAR, wants SUI - will trade his HBAR for Account 1's SUI
      { 
        finId: 'account2@atomic-swap.demo',
        name: 'Account 2 (Trader: HBAR → SUI)', 
        email: 'account2@atomic-swap.demo',
        wallets: {
          sui: account2SuiAddress, // Account 2's SUI wallet (wants to receive SUI)
          hedera: account2HederaAccount, // Account 2's HBAR wallet (has HBAR to trade)
          ethereum: process.env.SEPOLIA_WALLET_ADDRESS || '0x000...placeholder'
        }
      }
    ];
    ...
    this.logger.info(`   Account 1 has SUI in wallet: ${account1SuiAddress ? `${account1SuiAddress.substring(0, 10)}...` : 'not-available'}`);
    this.logger.info(`   Account 1 wants HBAR in wallet: ${account1HederaAccount}`);
    this.logger.info(`   Account 2 has HBAR in wallet: ${account2HederaAccount}`);
    this.logger.info(`   Account 2 wants SUI in wallet: ${account2SuiAddress ? `${account2SuiAddress.substring(0, 10)}...` : 'not-available'}`);
    this.logger.info('');
    this.logger.info('🎯 Trading Party Details:', {
      'account1@atomic-swap.demo': { 
        sui_wallet: account1SuiAddress ? `${account1SuiAddress.substring(0, 10)}...` : 'not-available',
        hedera_wallet: account1HederaAccount,
        trade: 'Has SUI → Wants HBAR'
      },
      'account2@atomic-swap.demo': { 
        sui_wallet: account2SuiAddress ? `${account2SuiAddress.substring(0, 10)}...` : 'not-available',
        hedera_wallet: account2HederaAccount,
        trade: 'Has HBAR → Wants SUI'
      }
    });

    this.logger.info('💡 Cross-Chain Asset Exchange Flow:');
    this.logger.info('   1. Account 1 queries FinP2P: "What is Account 2\'s SUI wallet address?"');
    this.logger.info('   2. Account 2 queries FinP2P: "What is Account 1\'s HBAR wallet address?"');
    this.logger.info('   3. Atomic swap execution:');
    this.logger.info('      → Account 1 sends SUI to Account 2\'s SUI wallet');
    this.logger.info('      → Account 2 sends HBAR to Account 1\'s HBAR wallet');
    this.logger.info('   4. Result: Account 1 gets HBAR, Account 2 gets SUI - TRUE ASSET EXCHANGE!');
    ...
    // Initial ownership (Account 1 owns some assets)
    this.mockOwnership.set('finp2p_account1_123:asset_sui_token_1', {
      finId: 'finp2p_account1_123',
      assetId: 'asset_sui_token_1',
  ...
  public async start(): Promise<void> {
    ...
    try {
      ...
      if (!this.config.mockMode) {
        ...
        this.logger.info('🎭 Running in mock mode');
      }
