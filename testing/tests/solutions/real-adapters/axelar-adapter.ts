...
export class AxelarAdapter extends EventEmitter {
  ...
  private signer: any | null = null; // Changed type to any to accommodate the new signer structure
  ...
  constructor(config: any) {
    ...
    const environment = config.environment || Environment.TESTNET;
      ...
      const testnetConfig = {
        ...
        chainId: 'axelar-testnet-lisbon-3' // Correct testnet chain ID
         ...
         this.assetTransfer = new AxelarAssetTransfer(testnetConfig);
     this.queryAPI = new AxelarQueryAPI(testnetConfig);
    ...
    this.initializeProviderAndSigner();
     ...
     this.supportedChains = [
       CHAINS.TESTNET.SEPOLIA,        // 'ethereum-sepolia' (11155111) - SUPPORTED
       CHAINS.TESTNET.BASE_SEPOLIA,   // 'base-sepolia' - SUPPORTED
       CHAINS.TESTNET.ARBITRUM_SEPOLIA, // 'arbitrum-sepolia' - SUPPORTED
       CHAINS.TESTNET.AVALANCHE,      // 'avalanche' - SUPPORTED
       CHAINS.TESTNET.BINANCE,        // 'binance' - SUPPORTED
       CHAINS.TESTNET.FANTOM,         // 'fantom' - SUPPORTED
       CHAINS.TESTNET.MOONBEAM,       // 'moonbeam' - SUPPORTED
       CHAINS.TESTNET.POLYGON         // 'polygon' - SUPPORTED
     ];
   }

           private initializeProviderAndSigner(): void {
      try {
        ...
        if (this.config.moonbeamUrl && this.config.moonbeamPrivateKey) {
          try {
            ...
            this.provider = new ethers.JsonRpcProvider(this.config.moonbeamUrl);
            
            // Create signer from private key
            this.signer = new ethers.Wallet(this.config.moonbeamPrivateKey, this.provider);
  ...
  async getAuditRecord(transferId: string): Promise<any> {
    try {
  ...
  getNetworkInfo(): any {
    return {
      name: 'Axelar Network',
      environment: this.config.environment || Environment.TESTNET,
