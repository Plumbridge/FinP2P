...
describe('Router Integration Tests', () => {
    ...
    afterAll(async () => {
        ...
        await (0, router_cleanup_1.stopRoutersSafely)([router1, router2, router3]);
    ...
    describe('Enhanced Balance Tracking Integration', () => {
        ...
        beforeEach(async () => {
            ...
            // Create test asset
            if (!mockAdapter1)
                throw new Error('Mock adapter 1 not found');
            ...
            if (!mockAdapter2)
                throw new Error('Mock adapter 2 not found');
        ...
        it('should handle cross-router balance validation', async () => {
            ...
            const validation1 = await ledgerManager1.validateBalanceAvailability('mock', account1Id, testAssetId, BigInt('150000000000000000000') // 150 tokens
            );
            expect(validation1.available).toBe(true);
            expect(validation1.available).toBe(true);
            // Validate balance on router2
            const validation2 = await ledgerManager2.validateBalanceAvailability('mock', account2Id, testAssetId, BigInt('80000000000000000000') // 80 tokens
            );
        ...
        it('should coordinate balance reservations across routers', async () => {
            ...
            const reservation1 = await ledgerManager1.reserveBalance('mock', account1Id, testAssetId, BigInt('50000000000000000000') // 50 tokens
            );
            expect(reservation1.success).toBe(true);
            // Reserve balance on router2
            const reservation2 = await ledgerManager2.reserveBalance('mock', account2Id, testAssetId, BigInt('30000000000000000000') // 30 tokens
            );
        ...
        it('should handle cross-router transfer operations with balance tracking', async () => {
            ...
            const crossTransfer = await ledgerManager1.initiateCrossLedgerTransfer('mock', 'mock', account1Id, account2Id, testAssetId, BigInt('25000000000000000000') // 25 tokens
            );
            ...
            if (!mockAdapter1)
                throw new Error('Mock adapter 1 not found');
            await mockAdapter1.transfer(account1Id, account2Id, testAssetId, BigInt('25000000000000000000'));
        ...
        it('should handle concurrent operations across multiple routers', async () => {
            ...
            if (!mockAdapter3)
                throw new Error('Mock adapter 3 not found');
            ...
            if (mockAdapter3) {
                await mockAdapter3.mintTokens(account3.finId.id, testAssetId, BigInt('75000000000000000000')); // 75 tokens
            }
        ...
        it('should maintain balance consistency during network partitions', async () => {
            ...
            await expect(ledgerManager1.validateBalanceAvailability('mock', account1Id, testAssetId, BigInt('10000000000000000000'))).rejects.toThrow('Network partition');
            // Restore network
            if (mockAdapter1) {
                mockAdapter1.simulateNetworkPartition(false);
            }
            // Operations should work again
            const validation = await ledgerManager1.validateBalanceAvailability('mock', account1Id, testAssetId, BigInt('10000000000000000000'));
            expect(validation.available).toBe(true);
            // Simulate balance reconciliation after partition
            if (mockAdapter1) {
                await mockAdapter1.simulateBalanceReconciliation(account1Id, testAssetId, BigInt('200000000000000000000') // Restore to original balance
                );
    ...
    beforeAll(async () => {
        ...
        global.testAccount1 = 'global-test-institution-1';
        global.testAccount2 = 'global-test-institution-2';
    });
    // Helper function to set up test accounts with balances
    const setupTestAccounts = async (router) => {
        const ledgerManager = router.getLedgerManager();
        // Create accounts if they don't exist
        try {
            const account1 = await ledgerManager.createAccount('mock', global.testAccount1);
            const account2 = await ledgerManager.createAccount('mock', global.testAccount2);
            ...
            const mockAdapter = ledgerManager.getAdapter('mock');
            await mockAdapter.mintTokens(global.testAccount1, 'USD', BigInt(1000000));
            await mockAdapter.mintTokens(global.testAccount2, 'USD', BigInt(1000000));
        }
    };
    beforeEach(async () => {
    ...
    describe('Transfer Processing', () => {
        ...
        it('should process a simple transfer between routers', async () => {
            ...
            const mockAdapter = router1.getLedgerManager().getAdapter('mock');
            await mockAdapter.mintTokens(account1.finId.id, 'USD', BigInt(1000000)); // 10,000 USD
            await mockAdapter.mintTokens(account2.finId.id, 'USD', BigInt(1000000)); // 10,000 USD
            const transfer = {
                id: 'transfer-001',
        ...
        it('should reject transfer when confirmation is denied', async () => {
            ...
            const result = await router1.processTransfer(transfer);
            expect(result.status).toBe(types_1.TransferStatus.FAILED);
        });
    });
    describe('Primary Router Authority', () => {
        ...
        it('should handle primary router authorization', async () => {
            ...
            const isAuthorized = await router1.checkPrimaryRouterAuthorization('USD', 'transfer', 'router-2');
        ...
        it('should deny unauthorized router access', async () => {
            const isAuthorized = await router1.checkPrimaryRouterAuthorization('USD', 'transfer', 'unauthorized-router');
    ...
    describe('Parallel Confirmation Processing', () => {
        ...
        it('should process multiple confirmations in parallel', async () => {
            ...
            for (let i = 0; i < 5; i++) {
                transfers.push({
                    id: `transfer-${i}`,
                    fromAccount: { id: testAccounts[i], type: 'account', domain: 'test.domain' },
                    toAccount: { id: testAccounts[i + 1], type: 'account', domain: 'test.domain' },
                    asset: { id: 'USD', type: 'asset', domain: 'test.domain' },
            ...
            const results = await Promise.all(transfers.map(transfer => router1.processTransfer(transfer)));
    ...
    describe('Network Communication', () => {
        ...
        it('should handle heartbeat messages', async () => {
            ...
            expect(knownRouters.some((r) => r.id === 'router-2')).toBe(true);
        });
    });
    describe('Error Handling and Resilience', () => {
        ...
        it('should handle adapter failures gracefully', async () => {
            ...
            const mockAdapter = router1.getLedgerAdapters()[0];
...
//# sourceMappingURL=RouterIntegration.test.js.map